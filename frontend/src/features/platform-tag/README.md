...
- **Spec**: `specs/005-platform-tag`
- **Responsibility**: 管理資源標籤、分類與治理規範
- **Architecture reference**: `grafana/public/app/features/tags`

## Data Closed-Loop

1. **Input**: 前端透過表單與設定面板輸入，或由 API 回傳原始資料。
2. **Processing**: `apiClient` 將請求送往 `/api/v1/platform-tag`，回應透過模組內的 hooks 與表格元件進行呈現。
