...
- **Spec**: `specs/014-dashboards-management`
- **Responsibility**: 治理儀表板庫、編輯器與戰情室
- **Architecture reference**: `grafana/public/app/features/dashboard`

## Data Closed-Loop

1. **Input**: 前端透過表單與設定面板輸入，或由 API 回傳原始資料。
2. **Processing**: `apiClient` 將請求送往 `/api/v1/dashboards-management`，回應透過模組內的 hooks 與表格元件進行呈現。
