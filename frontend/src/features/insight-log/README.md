...
- **Spec**: `specs/008-insight-log`
- **Responsibility**: 提供日誌分析、篩選與 AI 洞察
- **Architecture reference**: `grafana/public/app/features/logs`

## Data Closed-Loop

1. **Input**: 前端透過表單與設定面板輸入，或由 API 回傳原始資料。
2. **Processing**: `apiClient` 將請求送往 `/api/v1/insight-log`，回應透過模組內的 hooks 與表格元件進行呈現。
