# Feature Specification: Incidents List
...
### 3.2. 生命週期管理 (Lifecycle Management)
| 編號 | 說明 |
|------|------|
...
| 權限字串 | 描述 |
|-----------|------|
| `incidents:read` | 允許查看事件列表與詳情。 |
| `incidents:write` | 允許執行認領、指派、解決、靜音等操作。 |
| `incidents:comment` | 允許在事件上添加備註。 |
| `incidents:admin` | 允許刪除事件或修改他人備註。 |
