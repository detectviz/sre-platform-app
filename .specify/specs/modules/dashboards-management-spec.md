...
| **FR-021** | 使用者可將任何儀表板標記為「書籤 (Bookmark)」，以便於清單頂部快速存取。 |
| **FR-022** | 書籤狀態屬個人偏好設定，需與使用者 Profile 綁定並在登入後還原。 |
| **FR-023** | 系統應允許儀表板依下列方式排序：名稱 (A→Z)、建立時間、最近使用、書籤優先。 |
| **FR-024** | 使用者切換排序方式後，系統應記錄該偏好，於下次開啟頁面時自動套用。 |
| **FR-025** | 書籤狀態與排序設定不得影響全域顯示（即僅為個人層級設定）。 |
