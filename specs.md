...
-   (重用) `/dashboards/options`, `/settings/tags/options`: 被更多元件用於獲---

## 19. 功能: 事件的認領與指派 (Incident Acknowledge & Assignment Workflow)

-   **模組**: 事件管理 (Incident Management)
-   **版本**: v2.18
-   **狀態**: ✅ 已完成

### 19.1 使用者故事

**身為** 一名 SRE on-call 工程師或團隊主管，
**我想要** 在事件列表和詳情頁中，能夠清晰地看到每個事件的負責人，並能快速地認領（Acknowledge）一個新事件或將其轉派（Re-assign）給其他團隊成員，
**使得** 每個告警事件都有明確的所有權（Ownership），確保事件能被即時追蹤與處理，從而大幅提升團隊的事件響應效率與協作流暢度。

### 19.2 行為變更

-   **資料模型更新 (`types.ts`)**:
    -   `Incident` 介面中的 `assignee` 欄位已被修改為可選 (`assignee?: string`)。這是一個關鍵變更，允許新建立的事件在被認領前，處於真正的「無人指派」狀態。

-   **事件列表頁 (`IncidentListPage.tsx`)**:
    -   **動態「處理人 (Assignee)」欄位**:
        -   對於狀態為 `new` 且無 `assignee` 的事件，此欄位會顯示一個可點擊的**「👤 認領 (Acknowledge)」**按鈕。點擊後，系統會自動將當前登入的使用者設為處理人，並將事件狀態更新為 `acknowledged`。
        -   對於已指派的事件，此欄位會顯示處理人的姓名。點擊姓名會開啟一個新的**「轉派事件」模態框 (`AssignIncidentModal`)**，允許管理者或當前處理人將事件轉派給其他團隊成員。

-   **事件詳情頁 (`IncidentDetailPage.tsx`)**:
    -   **增強的「指派給」區塊**:
        -   對於無人處理的事件，此區塊會顯示「無」，並提供一個顯眼的**「✅ 確認此事件 (Acknowledge this Incident)」**按鈕，功能與列表頁的認領按鈕相同。
        -   對於已指派的事件，此區塊會顯示處理人的頭像和姓名，旁邊附有一個**「🔄 轉派 (Re-assign)」**按鈕，點擊後同樣會開啟轉派模態框。

-   **新增轉派模態框 (`AssignIncidentModal.tsx`)**:
    -   建立了一個新的專用模態框，用於處理事件的轉派。該模態框會動態從 `GET /iam/users` 端點獲取可用使用者列表，並以`下拉選單`呈現，讓轉派操作變得快速而準確。

### 19.3 使用的 API 端點

-   `POST /incidents/{id}/actions`: (增強) 此端點現在支援 `acknowledge` 和 `assign` 兩種新的 `action` 類型。
    -   `acknowledge`: 後端會將事件的 `status` 更新為 `acknowledged`，並將 `assignee` 設為當前操作的使用者。
