...
**身為** 一名 SRE 工程師，
**我想要** 在「日誌探索」頁面中，能夠一鍵觸發 AI 來總結當前篩選出的大量日誌，
**使得** 我可以快速地從海量數據中識別出關鍵的錯誤模式、異常趨勢和潛在的解決方案，而無需手動逐條閱讀。

### 15.2 行為變更

-   **UI 變更**:
    -   在「日誌探索」頁面 (`LogExplorerPage.tsx`) 的工具列中，新增了一個紫色的「AI 總結」按鈕。
-   **互動流程**:
    1.  使用者可以先透過搜尋框和時間範圍篩選出感興趣的日誌。
    2.  點擊「AI 總結」按鈕後，會開啟一個 `LogAnalysisModal` 模態框。
    3.  前端會向後端 `/ai/logs/summarize` 端點發起 `POST` 請求，並將當前的搜尋查詢 (`query`) 作為參數。
    4.  在等待 API 回應期間，模態框內會顯示載入中動畫。
    5.  收到 API 回應後，模態框會以結構化的形式展示 AI 生成的分析報告。
-   **結構化報告**: AI 分析報告不再是單純的文字，而是被劃分為三個清晰的區塊：
    -   **分析摘要**: 對日誌整體的總結。
    -   **偵測到的模式**: 列出 AI 識別出的主要日誌模式，包含日誌級別、描述和出現次數。
    -   **建議步驟**: 提供針對已發現問題的具體解決建議。

### 15.3 使用的 API 端點

-   `POST /ai/logs/summarize`: (新增) 用於對日誌進行 AI 分析與總結。
---

## 16. 功能: 分析中心進階功能實作 (Analysis Center Advanced Features)
...
**身為** 一名 SRE 工程師，
**我想要** 分析中心的功能是完整且數據驅動的，例如能夠從總覽頁面直接發起日誌搜尋，並在容量規劃頁面看到基於真實 API 數據的趨勢預測與建議，
**使得** 我可以無縫地在不同分析視圖之間切換，並根據準確的數據做出決策，提升問題排查和資源管理的效率。

### 16.2 行為變更

-   **分析總覽頁 (`AnalysisOverviewPage`)**:
    -   **API 數據驅動**: 頁面已完全移除前端的靜態模擬數據，所有圖表與日誌列表現在都透過向 `GET /api/v1/analysis/overview` 端點發起請求來動態載入。
    -   **日誌搜尋整合**: 頁面中的「Log Explorer」區塊的搜尋功能已啟用。使用者輸入查詢詞後點擊「Search」，會被導向至「日誌探索」頁面，並且搜尋詞會作為 URL 參數傳遞，自動填充到日誌探索頁的搜尋框中，立即執行查詢。

-   **日誌探索頁 (`LogExplorerPage`)**:
    -   **動態查詢載入**: 頁面現在能夠解析 URL 中的 `q` 查詢參數。如果存在，則會自動將其設為初始搜尋條件並載入對應的日誌數據，實現了從其他頁面的深度連結。

-   **容量規劃頁 (`CapacityPlanningPage`)**:
    -   **API 數據驅動**: 頁面已完全移除前端的靜態模擬數據，所有圖表與表格內容現在都透過向 `GET /api/v1/analysis/capacity-planning` 端點發起請求來動態載入。
    -   **動態圖表**: 「資源使用趨勢」和「容量預測模型」圖表現在完全基於 API 回傳的歷史與預測數據進行渲染。
    -   **AI 分析觸發**: 「觸發 AI 分析」按鈕的功能已實作。點擊後，會向後端重新請求最新的容量規劃數據，並在按鈕上顯示載入中狀態，為使用者提供即時的數據刷新能力。
    -   **健壯性提升**: 頁面增加了完整的載入中 (Loading) 與錯誤 (Error) 狀態處理，提升了非同步操作的使用者體驗。

### 16.3 使用的 API 端點

-   `GET /api/v1/analysis/overview`: (新增) 為分析總覽頁面提供所有必要的動態數據。
-   `GET /api/v1/analysis/capacity-planning`: (新增) 為容量規劃頁面提供所有必要的動態數據。
---

## 17. 功能: AI 鏈路追蹤瓶頸分析 (AI Trace Bottleneck Analysis)
...
**身為** 一名 SRE 工程師，
**我想要** 在「追蹤分析」頁面中，能夠一鍵觸發 AI 來分析選定鏈路的效能瓶頸，
**使得** 我可以快速地從複雜的服務調用中定位到延遲最高的環節，並獲得針對性的優化建議，從而顯著縮短問題排查時間。

### 17.2 行為變更

-   **UI 變更**:
    -   在「追蹤分析」頁面 (`TraceAnalysisPage.tsx`) 的工具列中，新增了一個紫色的「AI 分析瓶頸」按鈕。
    -   此按鈕僅在使用者選擇了一條追蹤鏈路後才會啟用。
-   **互動流程**:
    1.  使用者在左側列表中選擇一條感興趣的追蹤鏈路。
    2.  點擊「AI 分析瓶頸」按鈕後，會開啟一個 `TraceAnalysisModal` 模態框。
    3.  前端會向後端 `/ai/traces/analyze` 端點發起 `POST` 請求，並將選中的 `traceId` 作為參數。
    4.  在等待 API 回應期間，模態框內會顯示載入中動畫。
    5.  收到 API 回應後，模態框會以結構化的形式展示 AI 生成的分析報告。
-   **結構化報告**: 分析報告被劃分為三個清晰的區塊：
    -   **分析摘要**: 對追蹤鏈路整體效能的總結。
    -   **偵測到的瓶頸**: 列出 AI 識別出的主要效能瓶頸 Span，包含其服務名稱、操作名稱、耗時佔比和具體描述。
    -   **建議步驟**: 提供針對已發現瓶頸的具體解決建議。

### 17.3 使用的 API 端點

-   `POST /ai/traces/analyze`: (新增) 用於對指定的追蹤鏈路進行 AI 瓶頸分析。