...
### 10. 歷史數據回放 (Backtesting)

- **定位**: 提供驗證與模擬的能力，讓使用者能根據過去的歷史數據回放告警規則，檢查其正確性與合理性。
- **與告警引擎差異**:
  - 告警引擎 (Alert Engine): 聚焦於「即時數據流」，持續判斷當下是否符合觸發條件並立即通知。
  - 歷史數據回放 (Backtesting): 聚焦於「已存在的數據」，用於驗證規則在過去一段時間內是否會被觸發。
- **核心功能**:
  1. **回放規則驗證**: 使用者可選擇一組告警規則與一段歷史資料，系統模擬運行並輸出觸發次數、觸發點時間。
  2. **趨勢比較**: 提供可視化的「規則 vs. 數據」對照圖，標示哪些時間點會觸發告警。
  3. **誤報與漏報分析**: 使用者可標記「實際事件」，系統比對回放結果計算誤報率與漏報率。
  4. **策略調校支援**: 提供建議的門檻值與持續時間設定，幫助使用者調整規則以降低誤報。
  5. **批次模擬**: 支援一次回放多條規則，產生整體報告，用於審計與規則效能分析。
- **使用場景**:
  - 驗證新建立或修改過的告警規則，確保合理性。
  - 分析歷史事故，檢查當時規則是否能正確捕捉到事件。
  - 作為審計報告，提供規則設計與實際數據的對照證據。