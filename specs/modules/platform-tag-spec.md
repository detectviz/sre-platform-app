...
- **FR-008**：[NEEDS CLARIFICATION: `reference` 類型的標籤是如何工作的？它引用的是哪個物件的哪個欄位？其值的更新機制是什麼？]
- **FR-009**：[NEEDS CLARIFICATION: 標籤綱要的變更（例如，將一個選填標籤改為必填）對現有不合規的實體有何影響？是在下次編輯時強制要求補全，還是在報表中標示出來？]
...
| 項目 | 狀態 | 說明 |
|------|------|------|
| 記錄與追蹤 (Logging/Tracing) | ⚠️ | [NEEDS CLARIFICATION: 對標籤定義的所有 CRUD 操作以及其值的修改，是否都應產生詳細的審計日誌？這對治理至關重要。] |
...
- **[NEEDS CLARIFICATION: Schema Enforcement]** 需要詳細說明標籤綱要（如 `required`, `writable_roles`, `enum` 值）是如何在其他模組（如資源編輯頁）中被強制執行的。
- **[NEEDS CLARIFICATION: Reference Tag Logic]** 需要詳細定義 `reference` 類型標籤的行為、資料來源和更新機制。
- **[NEEDS CLARIFICATION: Auditing for Tags]** 需要確認對標籤綱要的所有變更是否都需要記錄審計日誌。
- **[NEEDS CLARIFICATION: Performance with Many Tags]** 雖然不太可能，但如果定義了大量標籤，前端篩選邏輯是否會有效能問題？（目前前端篩選，建議後端篩選）。