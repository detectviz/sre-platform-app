...
- **當 Grafana Proxy Token 失效**: 系統應自動刷新或提示重新授權。
- **當代理伺服器異常**: 顯示「Grafana 儀表板暫時無法連線」。
- **當非授權使用者存取代理路徑**: 返回 403 Forbidden 並寫入稽核日誌。
...
- **FR-012**: 所有外部 Grafana 請求必須經由平台後端 `/api/grafana/proxy/*` 進行安全轉發。
- **FR-013**: 代理層必須進行 RBAC 權限驗證與 Token 驗證，確保僅授權使用者可存取 Grafana 資源。
- **FR-014**: 每次代理請求必須自動記錄稽核事件（包含 user_id、dashboard_id、timestamp）。
...
- **SC-005**: 代理層延遲 ≤ 500ms，成功轉發率 ≥ 99%。
- **SC-006**: 代理稽核記錄完整率 100%，無遺漏事件。

---

## 外部 Grafana 安全代理模式

本系統採用安全代理（Proxy-based Integration）策略，所有對外部 Grafana 的請求均透過平台後端代理路徑 `/api/grafana/proxy/*` 進行轉發。此設計確保：

- 請求經由平台統一管控，避免直接暴露 Grafana API Key 或其他敏感資訊。
- 代理層執行嚴格的 RBAC 權限驗證與 Token 驗證，確保只有授權使用者能存取 Grafana 資源。
- 代理請求自動產生稽核紀錄，包含 user_id、dashboard_id 及請求時間戳，方便後續安全審計與問題追蹤。
- 當代理 Token 失效時，系統會自動刷新或提示使用者重新授權，提升使用體驗與安全性。
- 代理伺服器異常時，使用者將收到明確錯誤提示「Grafana 儀表板暫時無法連線」。
- 非授權使用者嘗試存取代理路徑時，系統將返回 403 Forbidden 並記錄稽核日誌。

此模式強化了整合的安全性與可管理性，並確保符合企業治理要求。
