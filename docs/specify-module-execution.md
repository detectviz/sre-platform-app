...
*   **治理檢查點**:
    ...
    *   **技術中立性**: 規格中嚴禁出現如 `JWT Bearer Token`、`React Context` 等技術實現細節。應專注於「什麼」而非「如何」。例如，描述「系統需提供一種機制以驗證使用者身份」，而非「使用 JWT 進行驗證」。
*   **建議行動**:
...
*   **建議行動**:
    *   為 `notification-management` 建立一個 `contracts/` 目錄，定義清晰的通知通道提供者介面，確保技術中立性和可擴展性。
