...
import { LayoutWidget, KpiDataEntry } from '@/shared/types';
...
const PageKPIs: React.FC<PageKPIsProps> = ({ pageName, widget_ids: explicit_widget_ids }) => {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 mb-8">
        <div className="flex flex-col items-center justify-center rounded-lg border border-slate-700 bg-slate-900/40 py-4 text-slate-400 space-y-2 min-h-[120px]">
  ...
  if (widget_ids.length === 0) {
    return (
      <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 mb-8">
        <div className="rounded-lg border border-dashed border-slate-700 bg-slate-900/40 p-4 text-center text-slate-400 min-h-[120px] flex flex-col items-center justify-center">
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 mb-8">
      {widget_ids.map(id => {
        ...
        if (!widget || !data) {
          return (
            <div key={id} className="flex flex-col items-center justify-center border border-dashed border-slate-700 bg-slate-900/40 rounded-lg p-3 text-center text-slate-400 space-y-1 min-h-[120px]">
