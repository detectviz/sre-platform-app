...
2.  **事件 (Events)**: 問題的核心，告警和事故的處理中心。
3.  **資源 (Resources)**: 被監控對象的清單和狀態檢視。
4.  **儀表板 (Dashboard)**: 統一的系統監控與業務洞察儀表板入口。
5.  **智慧排查 (Intelligent Investigation)**: 趨勢預測與深度洞察。
6.  **自動化 (Automation)**: 提升效率的工具和紀錄。
7.  **設定 (Settings)**: 平台的底層配置。

#### **3.2. 中樞頁面 (Hub Page) 設計模式**
每個主選單頁面（如事件、資源）都採用「中樞頁面」設計模式：
...
* **4.1.2. 混合儀表板模式**:
    ...
    * 在「儀表板」列表中，需明確標示儀表板類型，並提供差異化的操作（「編輯版面」vs「在 Grafana 中編輯」）。
* **4.1.3. 內建儀表板規劃**:
...
* **4.2.3. 告警規則 (Alert Rules)**:
    ...
    * **功能**:
        ...
        * **引導式建立/編輯**: 透過一個多步驟的精靈 (Wizard) 引導使用者完成規則建立。流程從選擇監控目標範本開始，逐步設定基本資訊、觸發條件、事件內容樣板，以及**可選的自動化響應腳本**。
