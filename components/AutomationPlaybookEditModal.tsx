        ...
        if (param.type === 'boolean') {
            return (
                <label className="flex items-center space-x-2">
                    ...
                    />
                    <span>{param.defaultValue ? 'Enabled' : 'Disabled'}</span>
                </label>
            );
        }
                    ...
                    <div className="flex-grow flex flex-col space-y-4 overflow-y-auto pr-2 -mr-4 pt-4 border-t border-slate-700/50">
                        <FormRow label="腳本內容">
                            <div className="relative">
                                ...
                                />
                                <button onClick={() => setIsAIOpen(true)} className="absolute top-2 right-2 flex items-center text-xs font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-md px-3 py-1.5">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-white mb-2">參數定義</h3>
                            <div className="space-y-3">
                                                ...
                                                <div className="space-y-2">
                                                    {param.options?.map((opt, optIndex) => (
                                                        <div key={optIndex} className="flex items-center space-x-2">
                                                            <input type="text" placeholder="Value" value={opt.value} onChange={e => handleOptionChange(index, optIndex, 'value', e.target.value)} className="w-1/2 bg-slate-700 rounded px-2 py-1 text-xs" />
                                                            <input type="text" placeholder="Label" value={opt.label} onChange={e => handleOptionChange(index, optIndex, 'label', e.target.value)} className="w-1/2 bg-slate-700 rounded px-2 py-1 text-xs" />
                                                    ...
                                                    <button onClick={() => addOption(index)} className="text-xs text-sky-400 hover:text-sky-300">+ Add Option</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <button onClick={addParam} className="text-sm text-sky-400 hover:text-sky-300 flex items-center mt-2">
                                    <Icon name="plus" className="w-4 h-4 mr-1" /> 新增參數
                                </button>
                            </div>
                        </div>
                    </div>
